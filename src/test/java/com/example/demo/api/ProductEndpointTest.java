...
public class ProductEndpointTest extends DemoApplicationTests {
  ...
  public void shouldCreateProduct() {
    ...
    ResponseEntity<ProductResponseDto> response =
        httpClient.postForEntity(baseUrl(), getHttpRequest(requestJson), ProductResponseDto.class);

    // then
    assertThat(response.getStatusCodeValue()).isEqualTo(201);
    ProductResponseDto rdto = response.getBody();
    assertThat(rdto.getName())
        .isEqualTo("testname"); // only name checking here, better implementation in Spock tests
  }
  ...
  public void shouldGetExistingProductsByTags() {
    ...
    // when
    ResponseEntity<ProductListResponseDto> result =
        httpClient.getForEntity(url, ProductListResponseDto.class);
    Set<String> fetchedProductsNames =
        result.getBody().getProducts().stream()
  ...
  public void shouldGetExistingProducts() {
    ...
    // when
    ResponseEntity<ProductListResponseDto> result =
        httpClient.getForEntity(baseUrl(), ProductListResponseDto.class);
  ...
  public void shouldUpdateExistingProduct() {
    ...
    // when
    String requestJson =
        mapToJson(new ProductRequestDto(newName, newPrice, newImage, newDescription, newTags));
    ResponseEntity<ProductResponseDto> reponse =
        httpClient.exchange(
            url, HttpMethod.PUT, getHttpRequest(requestJson), ProductResponseDto.class);
