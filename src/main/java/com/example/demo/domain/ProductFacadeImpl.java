...
class ProductFacadeImpl implements ProductFacade {
  ...
  public ProductListResponseDto getProductsByTags(List<String> tags) {
    List<Product> fetchedProducts =
        (tags == null ? repository.getAll() : repository.getByTags(tags));
    return new ProductListResponseDto(
        fetchedProducts.stream().map(this::productToResponseDto).collect(Collectors.toList()));
  }
  ...
  private Product createProductFromRequestDto(String id, ProductRequestDto dto) {
    return new ProductBuilder()
        ...
        .setTags(tagDtosToTags(dto.getTags()))
        .build();
  }
  ...
  private ProductResponseDto productToResponseDto(Product product) {
    return new ProductResponseDtoBuilder()
        ...
        .setPrice(
            new PriceDto(
                product.getPrice().getAmount().toString(),
                product.getPrice().getCurrencyCode().toString()))
